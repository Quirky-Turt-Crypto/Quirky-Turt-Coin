...
class ReorgStakeTest(quirkturtTestFramework):
    ...
    def check_money_supply(self, expected_qrtc):
        # verify that nodes have the expected QRTC supply
        qrtc_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(qrtc_supply, [DecimalAmt(expected_qrtc)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that QRTC supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check QRTC supply...")
