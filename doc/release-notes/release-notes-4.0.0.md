quirkyturt Core version *v4.0.0* is now available from:  <https://github.com/quirkyturt-project/quirkyturt/releases>
...
quirkyturt Core v4.0.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
You can read more details about this extensive work in ([PR #954](https://github.com/quirkyturt-Project/quirkyturt/pull/954))
...
A brand new feature is being introduced with the release of v4.0.0: Cold Staking ([PR #955](https://github.com/quirkyturt-Project/quirkyturt/pull/955))! This feature allows a coin owner to keep coins in a "cold" (or locked) wallet whilst a "hot" wallet carries out the burden of staking those coins.
...
A full technical writup is available on the [quirkyturt Wiki](https://github.com/quirkyturt-Project/quirkyturt/wiki/ColdStaking), and an initial video showcase is available on [YouTube](https://www.youtube.com/watch?v=utxB5TzAeXc).
A brief guide to setup cold staking with GUI and RPC is available [here](https://github.com/random-zebra/quirkyturt-Wiki/blob/master/User-Documentation/Cold-Staking-HowTo.md).

<sup>1</sup> Spending cold stakes from HW wallets currently available only for Ledger devices via [PET4L](https://github.com/quirkyturt-Project/PET4L) tool.

Multi-Split Stake Splitting
-------------------

Stake splitting has received a makeover and now supports splitting to more than two (2) outputs. [PR #968](https://github.com/quirkyturt-Project/quirkyturt/pull/968) introduced the change, which is controlled by the wallet's `stakesplitthreshold` setting.
...
Since the discovery of a critical exploit within the libzerocoin library in early 2019, remaining legacy v1 zQRTC have been un-spendable. We're happy to say that, once the new consensus rules are in effect, users will once again be able to spend their v1 zQRTC with public spends version 4 ([PR #936](https://github.com/quirkyturt-Project/quirkyturt/pull/936)).

As with the previous version 3 public spends introduced in core wallet version 3.3.0 (enabling the spending of v2 zQRTC), version 4 spends will also be public. A full technical writeup is available on the [quirkyturt Wiki](https://github.com/quirkyturt-Project/quirkyturt/wiki/CoinRandomnessSchnorrSignature).
...
[#PR1022](https://github.com/quirkyturt-Project/quirkyturt/pull/1022) defines Version 7 blocks, which remove the (now-unused) accumulator checkpoint from the block header. This results in an overall data reduction of ~256 bits from each block as well as the in-memory indexes.

### New Network Message Signatures

Layer 2 network messages (MN, Budget, Spork, etc) are now signed based on the hash of their **binary** content instead of their **string** representation ([#PR1024](https://github.com/quirkyturt-Project/quirkyturt/pull/1024)).

### New SPORKS

Two new SPORKS are introduced, `SPORK_17` ([#PR975](https://github.com/quirkyturt-Project/quirkyturt/pull/975)) and `SPORK_18` ([#PR995](https://github.com/quirkyturt-Project/quirkyturt/pull/995)).<br>
  ...
  "vout" : [              (array of json objects)
     {
       ...
       "scriptPubKey" : {          (json object)
         ...
         "addresses" : [           (json array of string)
           "quirkyturtaddress"        (string) quirkyturt address
           ,...
         ]
       }
     }
...
For our linux users, in addition to the [Ubuntu PPA](https://launchpad.net/~quirkyturt) repository, we are now offering a [Snap package](https://snapcraft.io/quirkyturt-core) as quick way to install and update a quirkyturt wallet.
...
- #1129 `a87bfc32a0` [Consensus] Define TestNet changeover block for quirkyturt v4.0 RC (random-zebra)
- #1134 `a4ded20de4` [Trivial] Remove a duplicate variable definition (warrows)
- #1191 `9a054eeba6` [Consensus] Define MainNet changeover block for quirkyturt v4.0 (random-zebra)
...
- #1125 `c6e238ca4d` [GUI] Inform if open quirkyturt.conf and/or backups folder fail. (furszy)
...
- #1179 `7a33fe10d2` [Qt] Fix for dead link to wrong quirkyturt website (NoobieDev12)
