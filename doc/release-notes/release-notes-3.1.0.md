quirkyturt Core version *3.1.0* is now available from:  <https://github.com/quirkyturt-project/quirkyturt/releases>
...
quirkyturt Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zQRTC Staking is here! zQRTC staking will be activated on the morning of the 8th of May 2018. With the release of zQRTC staking, there are effectively 2 versions of zQRTC, zQRTC minted on the 3.0.6 quirkyturt wallet or lower, and zQRTC minted on quirkyturt wallet version or higher. New features in this release will require the use of zQRTC v2, zQRTC minted on this wallet release 3.1.0 or later. If you currently hold zQRTC v1 and wish to take advantage of zQRTC staking and deterministic zQRTC, you will need to spend the zQRTC v1 to yourself and remint zQRTC v2.
...
To encourage the use of zQRTC and increase the quirkyturt zerocoin anonymity set, the quirkyturt payment schedule has been changed to the following:
...
The visual layout of the quirkyturt Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zQRTC minting feature. This is enabled by default and the enablezeromint=0 setting in the quirkyturt.conf file will overwrite the GUI option.
...
### Summary
 ...
 The quirkyturt wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the quirkyturt.conf file, found in the quirkyturt data directory.
...
quirkyturt Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The quirkyturt client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running quirkyturtd without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the quirkyturt support group, which often times sees users say "I didn't spend that zQRTC". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the quirkyturt masternodes, the monthly budget available to be utilised has been increased to 42,000 QRTC / month. This QRTC only has the opportunity to be raised once per month (paid to winning proposals) with any unused QRTC not created by the blockchain.

### Budget Finalization Fee

The quirkyturt finalization fee for successful proposals has now been reduced, this fee is now 5 QRTC down from 50 QRTC. The total fee outlay for a successful proposal is now a total of 55 QRTC.
...
-	Fixed a  bug that would not start masternodes from the quirkyturt-Qt masternodes tab
-	Updated quirkyturt-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and quirkyturt's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zQRTC Control dialog in the QT wallet, a user is able to see zQRTC marked as version 1 or 2.
...
quirkyturt's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which quirkyturt used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to quirkyturt using the bug bounty program. The modulus is now correctly imported and quirkyturt's accumulators have been changed to use the new proper modulus.
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault quirkyturt-cli getinfo while loading block index (Mrs-X)
