quirkyturt Core version *v5.0.0* is now available from:  <https://github.com/quirkyturt-project/quirkyturt/releases>
...
quirkyturt Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
#### Sapling Parameters
In order to run, quirkyturt Core now requires two files, `sapling-output.params` and `sapling-spend.params` (with total size ~50 MB), to be saved in a specific location.
...
For the other packages, the user must save the param files in the proper location, before being able to run quirkyturt v5.0.0:
- macOS/Linux `tar.gz` tarballs include a bash script (`install-params.sh`) to copy the parameters in the appropriate location.
- Windows `.zip` users need to manually copy the files from the `share/quirkyturt` folder to the `%APPDATA%\quirkyturtParams` directory.
...
More visual information about the protocol can be found at https://quirkyturt.org .
...
See the release notes from [v4.2.0](https://github.com/quirkyturt-Project/quirkyturt/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getaccountaddress` | Returned the current quirkyturt address associated with an internal account | Account system removal |
| `setaccount` | Associate a quirkyturt address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given quirkyturt address | Account system removal |
| `getaddressesbyaccount` | Returned a list of quirkyturt addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a quirkyturt address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a quirkyturt address | Pending a full rewrite |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_quirkyturt unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake quirkyturtd and unit tests: fixing missing MacOS framework. (furszy)
