...
#include "qt/quirkturt/splash.h"
#include "qt/quirkturt/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("quirkturt-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in quirkturt.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in quirkturt.qrc)
...
/** Class encapsulating quirkturt Core startup and shutdown.
...
/** Main quirkturt application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (quirkturtGUI)
...
{
    window = new quirkturtGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &quirkturtGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &quirkturtGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(quirkturtGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(quirkturtGUI::DEFAULT_WALLET);
        }
#endif
        ...
        // quirkturt: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &quirkturtGUI::handlePaymentRequest);
        connect(window, &quirkturtGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. quirkturt can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(quirkturt_locale);
    Q_INIT_RESOURCE(quirkturt);
    ...
    /// 6. Determine availability of data and blocks directory and parse quirkturt.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("quirkturt Core"),
            QObject::tr("Error: Specified data directory \"%1\" does not exist.").arg(QString::fromStdString(gArgs.GetArg("-datadir", ""))));
        return 1;
    }
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", quirkturt_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(0, QObject::tr("quirkturt Core"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("quirkturt Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // quirkturt: links repeatedly have their payment requests routed to this process:
