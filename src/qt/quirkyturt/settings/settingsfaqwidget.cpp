...
#include "qt/quirkyturt/settings/settingsfaqwidget.h"
#include "qt/quirkyturt/settings/forms/ui_settingsfaqwidget.h"
#include <QScrollBar>
#include <QMetaObject>
#include "qt/quirkyturt/qtutils.h"

SettingsFaqWidget::SettingsFaqWidget(quirkyturtGUI *parent) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({
           ...
           ui->labelNumber_UnspendableQRTC,
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableQRTC,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableQRTC,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("quirkyturt is a form of digital online money using blockchain technology "
        ...
        formatFAQParagraph(
            tr("quirkyturt utilizes a Proof of Stake (PoS) consensus system algorithm, "
    ...
    QString unspendableQRTCContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received quirkyturt requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your quirkyturt wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableQRTC->setText(unspendableQRTCContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of quirkyturt with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("quirkyturt-core", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.quirkyturt.org'>" + tr("#support in our Discord") + "</a>.")));
    ui->labelContent_Support->setText(supportContent);

    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node quirkyturt core wallet with a "
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in quirkyturt Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the quirkyturt network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("10,000 QRTC per single Masternode instance")) +
    ...
    QString mNControllerContent = formatFAQContent(
        formatFAQParagraph(
            tr("A Masternode Controller wallet is where the 10,000 QRTC collateral "
    ...
    connect(ui->pushButton_UnspendableQRTC, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableQRTC);});
    ...
    if (parent)
        connect(parent, &quirkyturtGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
