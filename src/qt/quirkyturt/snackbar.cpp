...
#include "qt/quirkyturt/forms/ui_snackbar.h"
#include "qt/quirkyturt/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(quirkyturtGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &quirkyturtGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
