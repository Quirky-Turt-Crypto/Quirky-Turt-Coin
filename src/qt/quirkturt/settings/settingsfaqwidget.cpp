...
#include "qt/quirkturt/settings/settingsfaqwidget.h"
#include "qt/quirkturt/settings/forms/ui_settingsfaqwidget.h"
#include <QScrollBar>
#include <QMetaObject>
#include "qt/quirkturt/qtutils.h"

SettingsFaqWidget::SettingsFaqWidget(quirkturtGUI *parent) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("quirkturt is a form of digital online money using blockchain technology "
        ...
        formatFAQParagraph(
            tr("quirkturt utilizes a Proof of Stake (PoS) consensus system algorithm, "
    ...
    QString unspendableQRTCContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received quirkturt requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your quirkturt wallet also needs to be completely synchronized "
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of quirkturt with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("quirkturt-core", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.quirkturt.org'>" + tr("#support in our Discord") + "</a>.")));
    ui->labelContent_Support->setText(supportContent);

    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node quirkturt core wallet with a "
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in quirkturt Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the quirkturt network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    if (parent)
        connect(parent, &quirkturtGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
