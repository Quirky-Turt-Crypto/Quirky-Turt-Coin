...
#include "qt/quirkturt/forms/ui_snackbar.h"
#include "qt/quirkturt/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(quirkturtGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &quirkturtGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
