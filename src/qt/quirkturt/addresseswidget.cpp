...
#include "qt/quirkturt/forms/ui_addresseswidget.h"
#include "qt/quirkturt/addresslabelrow.h"
#include "qt/quirkturt/addnewaddressdialog.h"
#include "qt/quirkturt/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(quirkturtGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto qrtcAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(qrtcAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(qrtcAdd);
        if (walletModel->updateAddressBookLabels(qrtcAdd, label.toUtf8().constData(),
