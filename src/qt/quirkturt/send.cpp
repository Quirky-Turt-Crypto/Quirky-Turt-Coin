...
#include "qt/quirkturt/send.h"
#include "qt/quirkturt/forms/ui_send.h"
#include "qt/quirkturt/addnewcontactdialog.h"
#include "qt/quirkturt/qtutils.h"
#include "qt/quirkturt/sendchangeaddressdialog.h"
#include "qt/quirkturt/optionbutton.h"
#include "qt/quirkturt/sendconfirmdialog.h"
#include "qt/quirkturt/guitransactionsutils.h"
...
SendWidget::SendWidget(quirkturtGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a quirkturt URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-qrtc");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onQRTCSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onQRTCSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent QRTCs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield QRTCs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent QRTCs to shield."));
    }
}
...
void SendWidget::onQRTCSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto qrtcAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(qrtcAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(qrtcAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(qrtcAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
