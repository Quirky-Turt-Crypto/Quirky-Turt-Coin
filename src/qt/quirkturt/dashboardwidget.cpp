...
#include "qt/quirkturt/dashboardwidget.h"
#include "qt/quirkturt/forms/ui_dashboardwidget.h"
#include "qt/quirkturt/sendconfirmdialog.h"
#include "qt/quirkturt/txrow.h"
...
DashboardWidget::DashboardWidget(quirkturtGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquarePiv, "square-chart-qrtc");
    setCssProperty(ui->labelSquarezPiv, "square-chart-zqrtc");
    setCssProperty(ui->labelPiv, "text-chart-qrtc");
    setCssProperty(ui->labelZqrtc, "text-chart-zqrtc");
    ...
    setCssProperty(ui->labelAmountPiv, "text-stake-qrtc-disable");
    setCssProperty(ui->labelAmountZqrtc, "text-stake-zqrtc-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &quirkturtGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("QRTC")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isPiv = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZQRTC;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair QRTC, zQRTC
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            qrtc = (pair.first != 0) ? pair.first / 100000000 : 0;
            zqrtc = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalPiv += pair.first;
            chartData->totalZqrtc += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesPiv.append(qrtc);
        chartData->valueszPiv.append(zqrtc);

        int max = std::max(qrtc, zqrtc);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalPiv > 0 || chartData->totalZqrtc > 0) {
        setCssProperty(ui->labelAmountPiv, "text-stake-qrtc");
        setCssProperty(ui->labelAmountZqrtc, "text-stake-zqrtc");
    } else {
        setCssProperty(ui->labelAmountPiv, "text-stake-qrtc-disable");
        setCssProperty(ui->labelAmountZqrtc, "text-stake-zqrtc-disable");
    }
    forceUpdateStyle({ui->labelAmountPiv, ui->labelAmountZqrtc});
    ui->labelAmountPiv->setText(GUIUtil::formatBalance(chartData->totalPiv, nDisplayUnit));
    ui->labelAmountZqrtc->setText(GUIUtil::formatBalance(chartData->totalZqrtc, nDisplayUnit, true));
