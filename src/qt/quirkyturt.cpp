...
#include "qt/quirkyturt/splash.h"
#include "qt/quirkyturt/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("quirkyturt-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in quirkyturt.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in quirkyturt.qrc)
...
/** Class encapsulating quirkyturt Core startup and shutdown.
...
/** Main quirkyturt application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (quirkyturtGUI)
...
{
    window = new quirkyturtGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &quirkyturtGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &quirkyturtGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(quirkyturtGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(quirkyturtGUI::DEFAULT_WALLET);
        }
#endif
        ...
        // quirkyturt: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &quirkyturtGUI::handlePaymentRequest);
        connect(window, &quirkyturtGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. quirkyturt can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(quirkyturt_locale);
    Q_INIT_RESOURCE(quirkyturt);
    ...
    /// 6. Determine availability of data and blocks directory and parse quirkyturt.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("quirkyturt Core"),
            QObject::tr("Error: Specified data directory \"%1\" does not exist.").arg(QString::fromStdString(gArgs.GetArg("-datadir", ""))));
        return 1;
    }
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", quirkyturt_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(0, QObject::tr("quirkyturt Core"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("quirkyturt Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // quirkyturt: links repeatedly have their payment requests routed to this process:
