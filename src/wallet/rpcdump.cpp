...
UniValue importprivkey(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            "importprivkey \"quirkyturtprivkey\" ( \"label\" rescan fStakingAddress )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"quirkyturtprivkey\"      (string, required) The private key (see dumpprivkey)\n"
    ...
    if (fRescan) {
        pwalletMain->RescanFromTime(TIMESTAMP_MIN, reserver, true /* update */);
    }
...
UniValue importaddress(const JSONRPCRequest& request)
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid quirkyturt address or script");
        }
    }
    if (fRescan) {
        pwalletMain->RescanFromTime(TIMESTAMP_MIN, reserver, true /* update */);
...
UniValue importwallet(const JSONRPCRequest& request)
{
    ...
    bool fGood = true;
    {
        ...
        while (file.good()) {
            ...
            // Sapling keys
            // Let's see if the address is a valid quirkyturt spending key
    ...
    pwalletMain->RescanFromTime(nTimeBegin, reserver, false /* update */);
    pwalletMain->MarkDirty();
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "dumpprivkey \"quirkyturtaddress\"\n"
            "\nReveals the private key corresponding to 'quirkyturtaddress'.\n"
            "Then the importprivkey can be used with this output\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"quirkyturtaddress\"   (string, required) The quirkyturt address for the private key\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by quirkyturt %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
UniValue importmulti(const JSONRPCRequest& mainRequest)
{
    ...
    if (fRescan && fRunScan && requests.size()) {
        int64_t scannedTime = pwalletMain->RescanFromTime(nLowestTimestamp, reserver, true /* update */);
        pwalletMain->ReacceptWalletTransactions();
                ...
                if (scannedTime <= GetImportTimestamp(request, now) || results.at(i).exists("error")) {
                    ...
                    result.pushKV("error", JSONRPCError(RPC_MISC_ERROR,
                                                strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                                          ...
                                                          "caused by pruning or data corruption (see quirkyturtd log for details) and could "
...
UniValue bip38encrypt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "bip38encrypt \"quirkyturtaddress\" \"passphrase\"\n"
            "\nEncrypts a private key corresponding to 'quirkyturtaddress'.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"quirkyturtaddress\"   (string, required) The quirkyturt address for the private key (you must hold the key already)\n"
...
UniValue bip38decrypt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "bip38decrypt \"quirkyturtaddress\" \"passphrase\"\n"
...
UniValue importsaplingkey(const JSONRPCRequest& request)
{
    ...
    if (fRescan) {
        pwalletMain->ScanForWalletTransactions(pindexRescan, nullptr, reserver, true);
    }
